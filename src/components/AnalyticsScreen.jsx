...
export default function AnalyticsScreen({ onBackToHome }) {
  ...
  const handleChartInteraction = (e) => {
    ...
    // Safely extract clientX on mobile devices to prevent touch crash errors
  ...
  // Temp debugging log to verify chart dataset availability on mobile remote console
        ...
        >
          ...
          >
            ...
            >
              ...
              {/* Vertical Dashed reference line for active point - Native SVG line with CSS transitions */}
              <line 
                x1={activePoint.x} 
                y1={10} 
                x2={activePoint.x} 
              ...
              {/* Active Hover pulse vertex - Native SVG circle with CSS transitions */}
              <circle 
                cx={activePoint.x}
                cy={activePoint.y}
                r="6"
            ...
            {/* FLOATING TOOLTIP CARD - Rebuilt using standard HTML div with CSS animations */}
            <div 
              className="absolute bg-white rounded-xl shadow-md border border-gray-100 p-2 px-3.5 flex flex-col text-left pointer-events-none z-20 transition-all duration-200 ease-out"
              style={{
